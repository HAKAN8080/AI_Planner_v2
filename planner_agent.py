...
def trading_analiz(kup: KupVeri) -> str:
    ...
    Analiz Sırası:
    1. Şirket Toplamı (Bütçe, Cover, LFL, Brüt Kar Kıyaslaması)
    2. Kategori Konsantrasyonu (ilk 3)
    3. Top 10 Ürün Analizi (ciro + depo stok durumu)
    4. Kritik Durumlar (sadece büyük kategoriler - ciro payı >%2)
    """
    ...
    kucuk_kat_sayisi = len(kategoriler) - len(buyuk_kategoriler)
    
    sonuc.append(f"({kucuk_kat_sayisi} küçük kategori analiz dışı bırakıldı)\n")
    ...
    kritik_marj = [k for k in buyuk_kategoriler if k['ty_marj'] < k['ly_marj'] - 3]
    ...
    if kritik_butce:
        ...
        for kat in sorted(kritik_butce, key=lambda x: x['ciro_achieved'])[:5]:
            sonuc.append(f"   • {kat['kategori']}: Bütçe %{kat['ciro_achieved']:+.0f} (ciro payı %{kat['ciro_pay']:.1f})")
        sonuc.append("")
    ...
    if kritik_cover:
        ...
        for kat in sorted(kritik_cover, key=lambda x: x['ty_cover'], reverse=True)[:5]:
            sonuc.append(f"   • {kat['kategori']}: {kat['ty_cover']:.1f} hf → Stok eritme gerekli")
        sonuc.append("")
    
    if kritik_lfl:
        sonuc.append(f"🔴 CİDDİ KÜÇÜLME ({len(kritik_lfl)} kategori - LFL <-%20):")
        for kat in sorted(kritik_lfl, key=lambda x: x['lfl_ciro'])[:5]:
            sonuc.append(f"   • {kat['kategori']}: LFL Ciro %{kat['lfl_ciro']:+.0f}")
        sonuc.append("")
    ...
    if kritik_marj:
        ...
        for kat in sorted(kritik_marj, key=lambda x: x['ty_marj'] - x['ly_marj'])[:5]:
            ...
            sonuc.append(f"   • {kat['kategori']}: %{kat['ty_marj']:.0f} (GY: %{kat['ly_marj']:.0f}) → {degisim:+.1f} puan")
        sonuc.append("")
    ...
    if iyi_gidenler:
        ...
        for kat in sorted(iyi_gidenler, key=lambda x: x['ciro_achieved'], reverse=True)[:3]:
            sonuc.append(f"   • {kat['kategori']}: Bütçe %{kat['ciro_achieved']:+.0f}, LFL %{kat['lfl_ciro']:+.0f}, Marj %{kat['ty_marj']:.0f}")
    ...
    if kritik_butce:
        ...
        sonuc.append(f"\n📌 ÖNCELİK 1: {en_kotu['kategori']} - bütçenin %{abs(en_kotu['ciro_achieved']):.0f} altında")
